...
+ (UIImage *)GGAnimatedGIFNamed:(NSString *)name {
    ...
    if (scale > 1.0f) {
        ...
        if (data) {
            return [UIImage GGAnimatedGIFWithData:data];
        }
...
+ (UIImage *)GGAnimatedGIFWithData:(NSData *)data {
    if (!data) {
        return nil;
    }
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += [self GGFrameDurationAtIndex:i source:source];
...
+ (float)GGFrameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
